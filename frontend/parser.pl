...
pat_dict([Key-Value|Rest]) --> pattern(Key), ['=>'(_)], arg_list(Value), ([','(_)], pat_dict(Rest) | {Rest = []}).
...
pattern(pat_dict(D)) --> ['['(_)], (pat_dict(D) | ['~'(_)], {D = []}), [']'(_)].
...
if(if(Condition, If, Else)) --> ['if'(_)], (instructions(Condition), ['->'(_)] | {Condition = []}), instructions(If), (['else'(_)], instructions(Else), ['end'(_)] | ['end'(_)], {Else = []}).
